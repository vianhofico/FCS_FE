...
import { App, Card, Spin, Row, Col, Typography } from 'antd';
import { HeartFilled, ShopOutlined, ArrowLeftOutlined, ShoppingCartOutlined, DeleteOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';

import { wishlistApi } from '@/modules/order/api/wishlistApi';
import type { WishlistItem } from '@/shared/contracts/wishlistContract';
...
export default function WishlistPage() {
  ...
  const fetchWishlist = async () => {
    setLoading(true);
    try {
      const response = await wishlistApi.getWishlist({ page: 0, size: 20 });
                  ...
                  cover={
                    <div className="relative aspect-[3/4] overflow-hidden bg-bg-secondary">
                      ...
                      <div className="absolute top-4 left-4 z-10">
                        <GradeBadge grade={getConditionLabel()} />
                      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-baseline gap-2">
                      <Text className="text-lg font-black text-primary">
                        {(item.productSalePrice ?? item.salePrice ?? 0).toLocaleString()}₫
                      </Text>
                    </div>
                  </div>
                </Card>
